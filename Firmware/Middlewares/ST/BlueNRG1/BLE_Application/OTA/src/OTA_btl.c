...
/* Extended packet length: L2CAP connection interval for setting proper connection interval vs flash write operations */
          ...
          */
          ...
          if (checksum == att_data[0]){
             ...
             { 
               ...
               if ((att_data[NEEDS_ACK_INDEX] == 1) || (((receivedSeqNum+1)*DATA_PACKET_SIZE) >= imageSize)) /* ((receivedSeqNum+1) % notification_range) or (att_data[NEEDS_ACK_INDEX] == 1)*/
               { 
